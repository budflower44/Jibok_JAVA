...
public class Method연습 {
	...
	public static void main(String[] args) {
		 ...
		 *     - 정수형 : int, byte, short, log(접미사 l(L))
		 *     - 실수형 : double, float(접미사 f(F))
		 *     - 정수형(문자) : char(정수형, 아스키코드 문자표에 저장되어 있는 코드로 저장) : 1글자 저장
		 ...
		 * - 증감연산자 (++, --)
		 *   - 1씩 증가, 1씩 감소
		 *   - 위치에 따라서 증가가 먼저 되거나 출력이 먼저되거나 순서가 결정됨.
		 *   - i++ , ++i
		 ...
		 *   - continue : pass 하는 역할
		 *     조건이 맞을 떄 실행하지 않고, 다음 증감으로 이동
		 *     while문에서는 증감식이 실행문안에 포함되기 때문에 조건 안에서 증감식을 찾지 못하는 경우 무한루프에 빠질 수 있음.
		 *     
		 * 6. 배열
