...
public class LottoManager implements LottoProgram{
	...
	public void createLotto(Scanner scan) {
		...
		int []tmp = new int[6]; //입력받을 임시 배열 변수
	...
	// for문을 너무 많이 돌리면 무한루프에 돌게 될 가능성이 높음 (간결하고 알아보기 슆게 or 어려운데 알아보기 쉽게)
	...
	public void insertLotto(Scanner scan) {
		//당첨번호 수동 입력.
		System.out.println("로또당첨번호 입력.(중복x, 1~45범위, 숫자 7개, 보너스는 마지막)");
		int tmp[] = new int[7];
		for(int i=0; i<tmp.length; i++) {
			tmp[i] = scan.nextInt();
		}
		...
		if(isDuplicated(tmp)) {
			...
			if(lotto.length == cnt) {
				LottoE[] arr = new LottoE[lotto.length+50];
